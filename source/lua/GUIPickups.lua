...
kPickupTextureYOffsets["BuildSentry"] = 13
...
local function GetPickupTextureCoordinates(pickup)
    ...
    local yOffset
    for _, pickupType in ipairs(kPickupTypes) do
    
        if pickup:isa(pickupType) then

            yOffset = kPickupTextureYOffsets[pickupType]
            break
...
local kIconsTextureName = "ui/ns2_combatequipment/drop_icons.dds"
